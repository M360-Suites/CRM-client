...
export default function RolesAccess() {
  ...
  {isLoading ? (
    <div className="overflow-hidden border border-[#E8E8E8] rounded-[12px]">
      <table className="w-full">
        <thead className="bg-[#F5B7A3]">
          <tr>
            <th className="text-left px-6 py-3  lg:text-sm text-xs font-medium text-foreground">Name</th>
            <th className="text-left px-6 py-3  lg:text-sm text-xs font-medium text-foreground">Email</th>
            <th className="text-left px-6 py-3  lg:text-sm text-xs font-medium text-foreground">Role</th>
            <th className="text-end px-6 py-3  lg:text-sm text-xs font-medium text-foreground">Actions</th>
          </tr>
        </thead>
        <tbody>
    ...
    <div className="flex flex-col items-center gap-4 py-20 border border-[#E8E8E8] rounded-[12px]">
      <span className="text-base font-normal text-foreground">No staff added yet</span>
      <CustomDrawer
        label="Add Staff"
        trigger={
          <CustomButton
      ...
      >
        {(close) => <AddStaffForm onSuccess={() => close()} />}
      </CustomDrawer>
    </div>
  ) : (
        ...
        <tbody>
          {staffs?.map((staff) => (
            <tr key={staff._id} className="border-t border-[#E8E8E8]">
              <td className="px-6 py-4 text-sm font-normal text-foreground">{staff.display_name}</td>
              <td className="px-6 py-4 text-sm font-normal text-foreground">{staff.email}</td>
              <td className="px-6 py-4 text-sm font-normal text-foreground">{handleRoleDisplay(staff.role)}</td>
