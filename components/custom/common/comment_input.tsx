...
>(({ label, error, type, className = "", textArea, ...inputProps }, ref) => {
	...
	return (
		<div className="flex flex-col gap-2 font-inter w-full max-w-full">
			...
			<div
				className={`rounded-[10px] border border-[#D9E1E8] flex flex-row flex-1 justify-between items-center ${error ? "border-foundation-error-6" : "border-border"} bg-[#F2F7FB] px-2 py-2`}
			>
				{textArea ? (
					<textarea
