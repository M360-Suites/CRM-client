...
export function CustomDateTimePicker({
	...
	const spinnerBtn =
		"flex items-center justify-center w-8 h-6 rounded-md border border-border hover:bg-[#F2F7FB] text-foundation-gray-4 transition-colors cursor-pointer";
	...
	return (
		<div
			className="flex flex-col gap-2 font-inter w-full relative"
			ref={ref}
		>
			...
			<button
				type="button"
				onClick={handleOpen}
				className={`rounded-[10px] border flex flex-row justify-between items-center px-4 py-4.5 bg-[#F2F7FB] w-full text-left ${
			...
			{error && (
				<p className="text-foundation-error-6 text-xs">{error}</p>
			)}
											...
											(period) => (
												<button
													...
													className={`px-2 py-1.5 text-sm font-medium rounded-[8px] border transition-colors cursor-pointer ${
														ampm === period
															? "bg-[#F2F7FB] text-foreground border-[#00B3A6]"
