(function () {
	...
	if (!script) {
		console.error(
			"[lead-capture] Failed to locate configuration script tag",
		);
		return;
	}
	...
	const source = window.location.hostname || "website";
	const hostname = window.location.hostname;
	const tags = script.dataset.tags || "";
	const selector =
	...
	function serializeForm(form) {
		...
		elements.forEach((el) => {
			...
			if (!fieldName || el.type === "submit" || el.type === "button")
				return;
	...
	window.addEventListener(
		...
		function (e) {
			...
			if (debug)
				console.log("[lead-capture] Sending inbound lead →", payload);
