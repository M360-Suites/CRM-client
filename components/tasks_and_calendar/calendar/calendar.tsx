		...
		<div className="flex items-center justify-between flex-wrap gap-3 mb-4 pb-3 border-b border-[#F3D9C4]">
			<div className="flex items-center gap-1.5">
				<button
					...
					className="text-xs font-medium px-3 py-1.5 rounded-full border border-border hover:bg-gray-100 transition-colors text-foreground"
				>
					Today
				</button>
				<button
			...
			<div className="flex items-center gap-1 border border-border rounded-full py-1 px-1">
				{viewList.map((v) => (
					<button
						key={v}
						onClick={() => onView(v)}
...
export default function CalendarView() {
	...
	const eventPropGetter: EventPropGetter<CalendarEvent> = (event) => ({
		style: {
			backgroundColor:
				PRIORITY_COLORS[event.resource?.priority] ?? "#4A9D8F",
