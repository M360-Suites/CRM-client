					...
					<div className="flex flex-row items-center gap-2  rounded-full py-2 md:px-1.75 bg-white shadow-[0px_6px_24px_0px_rgba(0,0,0,0.02),0px_0px_0px_1px_rgba(0,0,0,0.06)] px-1.5">
						{tabs.map((tab) => (
							<CustomButton
								...
								className={`rounded-full flex text-sm font-medium flex-row items-center gap-1.5 xl:px-4 xl:py-2 px-2.5 py-1.5 border-none ${activeTab === tab.value ? "bg-[#00B3A6] text-white" : "bg-transparent text-foreground hover:bg-gray-100"}`}
							>
								{tab.icon && (
									<tab.icon className="xl:size-4.5 size-4" />
								)}
								{tab.label}
							</CustomButton>
						))}
