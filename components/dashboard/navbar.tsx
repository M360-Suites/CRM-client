...
export default function Navbar() {
  ...
  return (
    <div className="fixed top-0 z-50 xl:px-10 lg:px-8 px-4 w-full md:py-1 py-1.5 flex items-center gap-3 border border-[#e8e8e8] bg-white ">
      ...
      <div className="  flex justify-between items-center w-full md:py-1 py-1.5">
        <Image
          src={CRMLOGO}
          alt="crm_logo"
          width={800}
        ...
        <div className="flex items-center gap-4.5">
          <div className="p-2.5 border rounded-full md:block hidden">
            <Bell
              color="#3A2418"
              className="lg:h-4 lg:w-4 md:w-3.5 md:h-3.5 "
            />
          </div>
          ...
          {isPending ? (
            ...
            <div className="flex items-center gap-2 p-2 animate-pulse">
              ...
              <div className="bg-gray-200 px-3 py-3 lg:h-10 lg:w-10 h-8 w-8 rounded-full" />
            </div>
          ) : (
            // real content
            <div className="flex items-center md:gap-2 gap-1 p-2">
              <div className="flex flex-col justify-center lg:gap-1 gap-0.5 items-end">
                <span className="lg:text-base/[120%]  text-sm font-medium capitalize">
                  {user?.display_name}
                </span>
                <span className="lg:text-sm/[120%] text-xs font-normal">
                  {user?.email}
                </span>
              </div>
              <div className="bg-[#F5B7A3] flex justify-center items-center lg:h-10 lg:w-10 h-9 w-9 lg:text-base text-sm font-bold text-foreground rounded-full">
